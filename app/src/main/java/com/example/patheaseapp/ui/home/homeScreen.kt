...
/*
 * Connect this list to your actual PathEase hazard data later.
 *
 * Example:
 * HazardPoint(
 *     location = LatLng(3.1234, 101.1234),
 *     level = SafetyLevel.DANGER,
 *     radiusMeters = 30.0
 * )
 *
 * SAFE = normal accessible path
 * CAUTION = uneven surface / minor obstacle
 * DANGER = blocked path / stairs / serious hazard
 */
private val hazardPoints = listOf(

    // SAFE areas don't need to be listed.
    // These are example danger zones.
...
private suspend fun getWalkingRoute(
    ...
    try {
        ...
        val distanceMeters =
            route.optInt("distanceMeters", 0)

        val durationText =
            route.optString("duration", "0s")
...
private fun getSafetyLevel(
    ...
    for (hazard in hazards) {
        ...
        if (distance <= hazard.radiusMeters) {
            if (hazard.level == SafetyLevel.DANGER) {
                return SafetyLevel.DANGER
            }

            if (hazard.level == SafetyLevel.CAUTION) {
                level = SafetyLevel.CAUTION
            }
        }
    }
...
fun HomeScreen(
    ...
    val apiKey = stringResource(
        id = com.example.patheaseapp.R.string.google_maps_key
    )
        ...
        var hasCenteredOnUser by remember {
            mutableStateOf(false)
        }

        var searchedPlace by remember {
            mutableStateOf<LatLng?>(null)
        }

        var searchedPlaceName by remember {
            mutableStateOf<String?>(null)
        }

        var searchedPlaceAddress by remember {
            mutableStateOf<String?>(null)
        }

        var walkingRoute by remember {
            mutableStateOf<WalkingRoute?>(null)
        }


        var routeStarted by remember {
            mutableStateOf(false)
        }
        ...
        var currentInstruction by remember {
            mutableStateOf("Follow the pedestrian route")
        }

        val isStarred = remember(searchedPlace, starredLocations) {
            starredLocations.any { 
                it.latitude == searchedPlace?.latitude && 
                it.longitude == searchedPlace?.longitude 
            }
        }
                                        ...
                                        if (
                                            ...
                                            coroutineScope.launch {
                                                ...
                                                if (route != null) {
                                                    ...
                                                    routeError =
                                                        "Unable to find a walking route."
                                                }
                                            }
                                        }
                                    },
                    ...
                    if (routeStarted && walkingRoute != null) {
                        Card(
                            ...
                            Column(
                                ...
                                Text(
                                    text =
                                        "Walking route • Check surroundings",
                                    modifier =
                                        Modifier.padding(
                                            top = 10.dp
                                        ),
...
private fun findInstructionForPoint(
    route: WalkingRoute,
    pointIndex: Int
): String {
    if (route.steps.isEmpty()) {
        return "Follow the pedestrian route"
    }

    var currentIndex = 0
    for (step in route.steps) {
        val stepSize = step.points.size
        if (pointIndex <= currentIndex + stepSize) {
            ...
            Activity.RESULT_OK -> {
                result.data?.let { data ->
                    ...
                    place.latLng?.let { latLng ->
                        onPlaceSelected(latLng, place.name, place.address)
                    }
                }
            }
